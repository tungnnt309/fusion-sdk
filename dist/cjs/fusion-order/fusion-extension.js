...
class FusionExtension {
    ...
    static fromExtension(extension) {
        ...
        const fees = new fees_1.Fees(new fees_1.ResolverFee(protocolFeeRecipient, interactionData.fees.resolverFee, interactionData.fees.whitelistDiscount), interactionData.fees.integratorFee.isZero()
            ? fees_1.IntegratorFee.ZERO
            : new fees_1.IntegratorFee(integratorFeeRecipient, protocolFeeRecipient, interactionData.fees.integratorFee, interactionData.fees.integratorShare));
        return new FusionExtension(settlementContract, auctionDetails, whitelist, surplusParams, {
            makerPermit,
            fees,
            customReceiver
    ...
    buildAmountGetterData(forAmountGetters) {
        ...
        const integrator = {
            fee: this.extra?.fees?.integrator.fee.toFraction(fees_1.Fees.BASE_1E5) || 0,
            share: this.extra?.fees?.integrator.share.toFraction(fees_1.Fees.BASE_1E2) ||
                0
        };
        const resolverFee = this.extra?.fees?.resolver.fee.toFraction(fees_1.Fees.BASE_1E5) || 0;
        ...
        builder
            ...
            whitelistDiscount.toFraction(fees_1.Fees.BASE_1E2)));
        if (forAmountGetters) {
    ...
    getFeesForTaker(taker) {
        const whitelistDiscount = this.extra?.fees?.resolver.whitelistDiscount.toFraction(fees_1.Fees.BASE_1E2) || 0;
        const discountNumerator = this.whitelist.isWhitelisted(taker)
            ? (Number(fees_1.Fees.BASE_1E2) - whitelistDiscount) /
                Number(fees_1.Fees.BASE_1E2)
            : 1;
        const resolverFee = discountNumerator *
            (this.extra?.fees?.resolver.fee.toFraction(fees_1.Fees.BASE_1E5) || 0);
        const resolverFeeBN = BigInt(resolverFee);
        const integratorFeeBN = BigInt(this.extra?.fees?.integrator.fee.toFraction(fees_1.Fees.BASE_1E5) || 0);
        return {
    ...
    getTakingAmountWithFee(taker, orderTakingAmount) {
        ...
        return (0, limit_order_sdk_1.mulDiv)(orderTakingAmount, fees_1.Fees.BASE_1E5 + fees.resolverFee + fees.integratorFee, fees_1.Fees.BASE_1E5, limit_order_sdk_1.Rounding.Ceil);
    }
}
...
function parseAmountData(iter) {
    const fees = {
        integratorFee: limit_order_sdk_1.Bps.fromFraction(Number(iter.nextUint16()), fees_1.Fees.BASE_1E5),
        integratorShare: limit_order_sdk_1.Bps.fromFraction(Number(iter.nextUint8()), fees_1.Fees.BASE_1E2),
        resolverFee: limit_order_sdk_1.Bps.fromFraction(Number(iter.nextUint16()), fees_1.Fees.BASE_1E5),
        whitelistDiscount: limit_order_sdk_1.Bps.fromFraction(Number(fees_1.Fees.BASE_1E2) - Number(iter.nextUint8()), fees_1.Fees.BASE_1E2)
    };
    return {
        fees
    };
